...
export function ChatList({
  ...
  if (!conversations || conversations.length === 0) {
    return (
      <div className="h-full max-h-full overflow-hidden flex flex-col">
        <h2 className="text-lg md:text-lg font-semibold p-4 md:p-6 border-b border-white/10 flex-shrink-0">Chats</h2>
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="text-center">
            <div className="text-6xl mb-4 opacity-50">💬</div>
            <p className="text-gray-400 text-base md:text-sm">No active conversations yet.</p>
            <p className="text-gray-500 text-sm md:text-xs mt-2">New chats will appear here</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="h-full max-h-full overflow-hidden flex flex-col">
      <h2 className="text-lg md:text-lg font-semibold p-4 md:p-6 border-b border-white/10 flex-shrink-0">
        Chats
        <span className="ml-2 text-sm text-gray-400 font-normal">({conversations.length})</span>
      </h2>
        ...
        <ul className="divide-y divide-white/5 md:divide-transparent">
          {conversations.map((convo) => (
            <li
              ...
              className={`min-h-[60px] md:h-20 px-4 md:px-6 py-4 cursor-pointer hover:bg-slate-700/50 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-colors duration-150 flex flex-col justify-center relative ${
              ...
              <div className="md:hidden">
                ...
                {convo.hasEscalation && (
                  <div className="flex items-center gap-2">
                    {convo.escalationStatus === 'pending' ? (
                      <span className="inline-flex items-center gap-1 text-yellow-500 text-sm font-medium bg-yellow-500/10 px-2 py-1 rounded">
                        ⚠️ Needs Attention
                      </span>
                    ) : convo.escalationStatus === 'attending' ? (
                      <span className="inline-flex items-center gap-1 text-green-500 text-sm font-medium bg-green-500/10 px-2 py-1 rounded">
                        ✓ You're Helping
                      </span>
                    ) : null}
                  </div>
