...
class MessageProcessor {
  ...
  private async executeFirstStep(userCurrentGoal: UserGoal, currentContext: ChatContext, incomingUserMessage: string): Promise<{ responseToUser: string; uiButtonsToDisplay?: ButtonConfig[] }> {
    const currentSteps = conversationFlowBlueprints[userCurrentGoal.flowKey];
    if (!currentSteps || !currentSteps[userCurrentGoal.currentStepIndex]) throw new Error('No handler found for first step');

    const stepName = currentSteps[userCurrentGoal.currentStepIndex];
    const firstStepHandler = botTasks[stepName];
    if (!firstStepHandler) throw new Error('No handler found for first step');
    ...
    const responseToUser = userCurrentGoal.collectedData.confirmationMessage || firstStepHandler.defaultChatbotPrompt || "Let's get started with your booking.";
    let uiButtonsToDisplay: ButtonConfig[] | undefined;
    if (firstStepHandler.fixedUiButtons) {
      uiButtonsToDisplay = typeof firstStepHandler.fixedUiButtons === 'function' ?
  ...
  private async executeAutoAdvanceStep(userCurrentGoal: UserGoal, currentContext: ChatContext): Promise<{ responseToUser: string; uiButtonsToDisplay?: ButtonConfig[] }> {
    const currentSteps = conversationFlowBlueprints[userCurrentGoal.flowKey];
    const stepName = currentSteps[userCurrentGoal.currentStepIndex];
    const stepHandler = botTasks[stepName];
    if (!stepHandler) throw new Error(`No handler found for auto-advance step: ${stepName}`);
    ...
    if ((stepHandler.autoAdvance || shouldConditionallyAdvance) && userCurrentGoal.currentStepIndex + 1 < currentSteps.length) {
      console.log(`[MessageProcessor] Auto-advancing from step: ${stepName}`);
      if (shouldConditionallyAdvance) userCurrentGoal.collectedData.shouldAutoAdvance = false;
      const targetStep = this.navigateToAppropriateStep(userCurrentGoal);
      const targetStepIndex = currentSteps.indexOf(targetStep);
      userCurrentGoal.currentStepIndex = targetStepIndex !== -1 ? targetStepIndex : userCurrentGoal.currentStepIndex;
      this.advanceAndSkipStep(userCurrentGoal);
      return await this.executeAutoAdvanceStep(userCurrentGoal, currentContext);
    }

    const responseToUser = userCurrentGoal.collectedData.confirmationMessage || stepHandler.defaultChatbotPrompt || "Continuing with your booking...";
    let uiButtonsToDisplay: ButtonConfig[] | undefined;
    if (stepHandler.fixedUiButtons) {
      uiButtonsToDisplay = typeof stepHandler.fixedUiButtons === 'function' ?
  ...
  private async processExistingGoalIntelligent(userCurrentGoal: UserGoal, currentContext: ChatContext, incomingUserMessage: string, customerUser?: {firstName: string, lastName: string, id: string}): Promise<{ responseToUser: string; uiButtonsToDisplay?: ButtonConfig[] }> {
    console.log(`[MessageProcessor] Processing existing goal with intelligent enhancement`);
    const knownButtonValues = ['choose_another_day', 'open_calendar', 'confirm_quote', 'edit_quote', 'edit_service', 'edit_time', 'tomorrow_7am', 'tomorrow_9am'];
    const isButtonClick = knownButtonValues.includes(incomingUserMessage) || incomingUserMessage.startsWith('slot_') || incomingUserMessage.startsWith('day_') || /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(incomingUserMessage);

    if (isButtonClick) {
      console.log(`[MessageProcessor] Detected button click: "${incomingUserMessage}" - using original blueprint flow`);
      return this.processOriginalFlowWithIntelligentEnhancement(userCurrentGoal, currentContext, incomingUserMessage, undefined, customerUser);
    }
    
    const messageHistory = userCurrentGoal.messageHistory.map(msg => ({ role: msg.speakerRole === 'user' ? 'user' as const : 'assistant' as const, content: msg.content, timestamp: msg.messageTimestamp }));

    try {
      const conversationDecision = await this.llmService.analyzeConversationFlow(incomingUserMessage, userCurrentGoal, currentContext, messageHistory);
      console.log(`[MessageProcessor] Intelligent analysis:`, conversationDecision);

      if (conversationDecision.action === 'go_back' && conversationDecision.confidence > 0.7) return this.handleGoBack(userCurrentGoal, currentContext, conversationDecision, customerUser);
      if (conversationDecision.action === 'restart' && conversationDecision.confidence > 0.8) return this.handleRestart(userCurrentGoal, currentContext);
      if (conversationDecision.action === 'switch_topic' && conversationDecision.confidence >= 0.8) {
        ...
        return { responseToUser: topicSwitchResult.responseToUser, uiButtonsToDisplay: topicSwitchResult.uiButtonsToDisplay };  
      }
      return this.processOriginalFlowWithIntelligentEnhancement(userCurrentGoal, currentContext, incomingUserMessage, conversationDecision, customerUser);
    } catch (error) {
  ...
  private async handleGoBack(userCurrentGoal: UserGoal, currentContext: ChatContext, conversationDecision: any, customerUser?: {firstName: string, lastName: string, id: string}): Promise<{ responseToUser: string; uiButtonsToDisplay?: ButtonConfig[] }> {
    ...
    if (targetStepHandler) {
      ...
      userCurrentGoal.collectedData = typeof targetStepResult === 'object' && 'extractedInformation' in targetStepResult ? { ...userCurrentGoal.collectedData, ...targetStepResult.extractedInformation } : targetStepResult as Record<string, any>;
      
      let responseToUser: string;
      try {
        const contextualResponse = await this.llmService.generateContextualResponse(userCurrentGoal, currentContext, 'User wants to go back and change something', conversationDecision, userCurrentGoal.messageHistory.map(msg => ({ role: msg.speakerRole === 'user' ? 'user' as const : 'assistant' as const, content: msg.content, timestamp: msg.messageTimestamp })), customerUser);
      ...
      if (targetStepHandler.fixedUiButtons) {
        uiButtonsToDisplay = typeof targetStepHandler.fixedUiButtons === 'function' ? await targetStepHandler.fixedUiButtons(userCurrentGoal.collectedData, currentContext) : targetStepHandler.fixedUiButtons;
      }
      return { responseToUser, uiButtonsToDisplay };
    } else {
  ...
  private async handleRestart(userCurrentGoal: UserGoal, currentContext: ChatContext): Promise<{ responseToUser: string; uiButtonsToDisplay?: ButtonConfig[] }> {
    console.log(`[MessageProcessor] Handling restart request`);
    userCurrentGoal.currentStepIndex = 0;
    userCurrentGoal.collectedData = { availableServices: userCurrentGoal.collectedData.availableServices };
    return this.executeFirstStep(userCurrentGoal, currentContext, "restart");
  }

  private async handleTopicSwitch(currentContext: ChatContext, conversationDecision: any, incomingUserMessage: string, currentGoal?: UserGoal): Promise<{ responseToUser: string; uiButtonsToDisplay?: ButtonConfig[]; newGoal?: UserGoal }> {
    ...
    if (conversationDecision.newGoalType === 'serviceBooking' && conversationDecision.newGoalAction === 'create') {
      ...
      const newGoal = await this.createNewGoal({ detectedUserGoalType: 'serviceBooking', detectedGoalAction: 'create', extractedInformation: conversationDecision.extractedData || {} }, 'customer', currentContext);
      const result = await this.executeFirstStep(newGoal, currentContext, incomingUserMessage);
      return { ...result, newGoal };
    }
    return { responseToUser: "I understand you'd like to switch topics. Let me help you with that.", uiButtonsToDisplay: [] };
  }

  private async processOriginalFlowWithIntelligentEnhancement(userCurrentGoal: UserGoal, currentContext: ChatContext, incomingUserMessage: string, conversationDecision?: any, customerUser?: {firstName: string, lastName: string, id: string}): Promise<{ responseToUser: string; uiButtonsToDisplay?: ButtonConfig[] }> {
    userCurrentGoal.messageHistory.push({ speakerRole: 'user', content: incomingUserMessage, messageTimestamp: new Date() });

    const currentSteps = conversationFlowBlueprints[userCurrentGoal.flowKey];
    if (!currentSteps || !currentSteps[userCurrentGoal.currentStepIndex]) throw new Error('No handler found for current step');
    ...
    if (isInputValid) {
      ...
      userCurrentGoal.collectedData = typeof processingResult === 'object' && 'extractedInformation' in processingResult ? { ...userCurrentGoal.collectedData, ...processingResult.extractedInformation } : processingResult as Record<string, any>;

      if (userCurrentGoal.collectedData.restartBookingFlow) return this.handleOriginalRestartFlow(userCurrentGoal, currentContext);
      if (userCurrentGoal.collectedData.navigateBackTo) return this.handleOriginalNavigateBack(userCurrentGoal, currentContext);
      ...
      if (userCurrentGoal.currentStepIndex >= currentSteps.length || userCurrentGoal.collectedData.goalStatus === 'completed') {
        ...
        if (nextStepHandler) {
          ...
          userCurrentGoal.collectedData = typeof nextStepResult === 'object' && 'extractedInformation' in nextStepResult ? { ...userCurrentGoal.collectedData, ...nextStepResult.extractedInformation } : nextStepResult as Record<string, any>;
          
          if (nextStepHandler.autoAdvance || userCurrentGoal.collectedData.shouldAutoAdvance) {
            if (userCurrentGoal.collectedData.shouldAutoAdvance) userCurrentGoal.collectedData.shouldAutoAdvance = false;
            const targetStep = this.navigateToAppropriateStep(userCurrentGoal);
            const targetStepIndex = currentSteps.indexOf(targetStep);
            userCurrentGoal.currentStepIndex = targetStepIndex !== -1 ? targetStepIndex : userCurrentGoal.currentStepIndex;
            this.advanceAndSkipStep(userCurrentGoal);
            ...
            try {
              if (conversationDecision) {
                const contextualResponse = await this.llmService.generateContextualResponse(userCurrentGoal, currentContext, incomingUserMessage, conversationDecision, userCurrentGoal.messageHistory.map(msg => ({ role: msg.speakerRole === 'user' ? 'user' as const : 'assistant' as const, content: msg.content, timestamp: msg.messageTimestamp })), customerUser);
                responseToUser = contextualResponse.text;
              } else {
                responseToUser = userCurrentGoal.collectedData.confirmationMessage || nextStepHandler.defaultChatbotPrompt || "Let's continue with your booking.";
              }
            } catch (error) {
            ...
            if (nextStepHandler.fixedUiButtons) {
              try {
                uiButtonsToDisplay = typeof nextStepHandler.fixedUiButtons === 'function' ? await nextStepHandler.fixedUiButtons(userCurrentGoal.collectedData, currentContext) : nextStepHandler.fixedUiButtons;
              } catch (error) {
      ...
      if (conversationDecision && (conversationDecision.action === 'continue' || conversationDecision.action === 'switch_topic') && conversationDecision.confidence > 0.7) {
        try {
          const contextualResponse = await this.llmService.generateContextualResponse(userCurrentGoal, currentContext, incomingUserMessage, conversationDecision, userCurrentGoal.messageHistory.map(msg => ({ role: msg.speakerRole === 'user' ? 'user' as const : 'assistant' as const, content: msg.content, timestamp: msg.messageTimestamp })), customerUser);
          responseToUser = contextualResponse.text;
          ...
          if (currentStepHandler.fixedUiButtons) {
            try {
              uiButtonsToDisplay = typeof currentStepHandler.fixedUiButtons === 'function' ? await currentStepHandler.fixedUiButtons(userCurrentGoal.collectedData, currentContext) : currentStepHandler.fixedUiButtons;
            } catch (error) {
        ...
        try {
          if (conversationDecision) {
            const contextualResponse = await this.llmService.generateContextualResponse(userCurrentGoal, currentContext, incomingUserMessage, { ...conversationDecision, action: 'continue' }, userCurrentGoal.messageHistory.map(msg => ({ role: msg.speakerRole === 'user' ? 'user' as const : 'assistant' as const, content: msg.content, timestamp: msg.messageTimestamp })), customerUser);
            responseToUser = contextualResponse.text;
          } else {
  ...
  private async handleOriginalRestartFlow(userCurrentGoal: UserGoal, currentContext: ChatContext) {
    ...
    if (targetStepHandler) {
      const targetStepResult = await targetStepHandler.processAndExtractData("", userCurrentGoal.collectedData, currentContext);
      userCurrentGoal.collectedData = typeof targetStepResult === 'object' && 'extractedInformation' in targetStepResult ? { ...userCurrentGoal.collectedData, ...targetStepResult.extractedInformation } : targetStepResult as Record<string, any>;
      const responseToUser = userCurrentGoal.collectedData.confirmationMessage || targetStepHandler.defaultChatbotPrompt || "Let's continue with your new service selection.";
      let uiButtonsToDisplay: ButtonConfig[] | undefined;
      if (targetStepHandler.fixedUiButtons) {
        uiButtonsToDisplay = typeof targetStepHandler.fixedUiButtons === 'function' ? await targetStepHandler.fixedUiButtons(userCurrentGoal.collectedData, currentContext) : targetStepHandler.fixedUiButtons;
      }
  ...
  private async handleOriginalNavigateBack(userCurrentGoal: UserGoal, currentContext: ChatContext) {
    ...
    if (targetStepHandler) {
      const targetStepResult = await targetStepHandler.processAndExtractData("", userCurrentGoal.collectedData, currentContext);
      userCurrentGoal.collectedData = typeof targetStepResult === 'object' && 'extractedInformation' in targetStepResult ? { ...userCurrentGoal.collectedData, ...targetStepResult.extractedInformation } : targetStepResult as Record<string, any>;
      const responseToUser = userCurrentGoal.collectedData.confirmationMessage || targetStepHandler.defaultChatbotPrompt || "Let's update your selection.";
      let uiButtonsToDisplay: ButtonConfig[] | undefined;
      if (targetStepHandler.fixedUiButtons) {
        uiButtonsToDisplay = typeof targetStepHandler.fixedUiButtons === 'function' ? await targetStepHandler.fixedUiButtons(userCurrentGoal.collectedData, currentContext) : targetStepHandler.fixedUiButtons;
      }
      ...
      if (nextStepHandler) {
        ...
        if (nextIsInputValid) {
          ...
          if (nextStepHandler.autoAdvance || userCurrentGoal.collectedData.shouldAutoAdvance) {
            ...
            const responseToUser = userCurrentGoal.collectedData.confirmationMessage || nextStepHandler.defaultChatbotPrompt || "Let's continue with your booking.";
            let uiButtonsToDisplay: ButtonConfig[] | undefined;
            if (nextStepHandler.fixedUiButtons) {
              uiButtonsToDisplay = typeof nextStepHandler.fixedUiButtons === 'function' ? await nextStepHandler.fixedUiButtons(userCurrentGoal.collectedData, currentContext) : nextStepHandler.fixedUiButtons;
            }
  ...
  private async persistUpdatedState(sessionId: string, userContext: UserContext, activeSession: ChatConversationSession, currentGoal: UserGoal | undefined, userMessage: string, botResponse: string): Promise<void> {
    try {
