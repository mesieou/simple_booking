...
// Calculates the travel cost based on business and pricing type.
export function calculateTravelCost(
  service: Service,
  business: Business,
  travelTimeEstimate: number
): number {
  // No travel cost for non-mobile businesses or fixed price services
  if (!business.mobile || service.pricingType === 'fixed') {
    return 0;
  }
...
export function calculateTotalJobDuration(
  ...
  if (business.mobile && service.pricingType === 'per_minute') {
...
// Calculates the total job cost, service cost, and travel cost for all business/pricing combinations.
export function calculateTotalJobCost(
  ...
  let travelCost = calculateTravelCost(service, business, travelTimeEstimate);
  let totalJobCost = serviceCost + travelCost;

  if (shouldApplyBaseCharge(service, business, totalJobCost)) {
    totalJobCost = service.baseCharge!;
...
// Computes all fields for the quote, using the above helpers.
export function computeQuoteEstimation(
  ...
  if (isMobilePerMinuteWithMissingTravelTime(service, business, travelTimeEstimate)) {
    throw new Error('travelTimeEstimate is required for mobile businesses with per-minute pricing.');
  }

  const { totalJobCost, serviceCost, travelCost } = calculateTotalJobCost(service, business, travelTimeEstimate ?? 0);
  const totalJobDuration = calculateTotalJobDuration(service, business, travelTimeEstimate ?? 0);
